...
///
/// Series color is `theme.colors.chartPalette[0]` — real shadcn's default
/// theme has a DEDICATED chart palette (`--chart-1` through `--chart-5`, all
/// shades of blue in the stock theme), separate from `--primary`, and now a
/// real token in `Tokens.swift` (was hardcoded locally here in an earlier
/// pass, before that pass owned `Tokens.swift`). An earlier version used
/// `theme.colors.primary`, which real shadcn never does for chart series.
/// `UI.Chart` only renders one series today, so only the first palette step
/// has a caller — `chartPalette[1...]` are ready for a future multi-series
/// chart without needing another token pass first.
///
...
public extension UI {
    ...
    struct Chart: View {
        ...
        // Real shadcn's `--chart-1` — see the type-level doc comment.
        private var seriesColor: Color { theme.colors.chartPalette[0] }
